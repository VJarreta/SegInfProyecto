...
public class Twofish {
    public static byte[] twofishEncrypt(byte[] plaintext, byte[] signingKey) {
        ...
        try {
            ...
            PaddedBufferedBlockCipher twofish = new PaddedBufferedBlockCipher(new CBCBlockCipher(new TwofishEngine()));
            CipherParameters ivAndKey = new ParametersWithIV(new KeyParameter(signingKey), iv);
            twofish.init(true, ivAndKey);
            byte[] output = new byte[twofish.getOutputSize(plaintext.length)];
            int ciphertextLength = twofish.processBytes(plaintext, 0, plaintext.length, output, 0);
            ciphertextLength += twofish.doFinal(output, ciphertextLength);
    ...
    public static byte[] twofishDecrypt(byte[] encrypted, byte[] signingKey) {
        if (signingKey.length != 32) {
            throw new IllegalArgumentException("Key length must be 32 bytes");
        }
        try {
            ...
            PaddedBufferedBlockCipher twofish = new PaddedBufferedBlockCipher(new CBCBlockCipher(new TwofishEngine()));
            CipherParameters ivAndKey = new ParametersWithIV(new KeyParameter(signingKey), iv);
            twofish.init(false, ivAndKey);
            byte[] output = new byte[twofish.getOutputSize(ciphertext.length)];
            int plaintextLength = twofish.processBytes(ciphertext, 0, ciphertext.length, output, 0);
            plaintextLength += twofish.doFinal(output, plaintextLength);
    ...
    public static void bucle() {
        ...
        try{
            ...
            while(j>0){
                ...
                cipherText = twofishEncrypt(input20k,keyBytes);

                // decryption pass

                byte[] plainText = twofishDecrypt(cipherText,keyBytes);
            ...
            for(int i=0; i<nTimes; i++){
                ...
                cipherText = twofishEncrypt(input100k,keyBytes);
                ...
                plainText = twofishDecrypt(cipherText,keyBytes);
                ...
                cipherText = twofishEncrypt(input200k,keyBytes);
            ...
            cipherText = twofishEncrypt(input50MB,keyBytes);
            ...
            System.out.println("Algorithm: Twofish/CBC/PKCS7Padding, Provider: BC");
